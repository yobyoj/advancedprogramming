...
//Deconstructor the instance
Rectangle::~Rectangle(){
}

//Sets data value of square
void Rectangle::setData(int *x, int *y){
    bool check = false;
    int i = 1;

    //Sets mininum_value_of_x & maximum_value_of_x
    while (check == false) {
        if (x[0] == x[i]){
            i++;
        }
    ...
    //Sets mininum_value_of_y & maximum_value_y
    while (check == false) {
        if (y[0] == y[i]){
            i++;
        }
    ...
    //Sets length and breath respectively
...
bool Rectangle::isPointInShape(int x, int y){
    //Parameter Checking: For Shape of Boundaries
...
string Rectangle::getPointsOnPerimeter(){
    string output = "";
...
string Rectangle::getPointsWithinShape(){
    string output = "";
    for (int i = mininum_value_of_x; i <= maximum_value_of_x;i++){
        for (int j = mininum_value_of_y; j <= maximum_value_y; j++){
